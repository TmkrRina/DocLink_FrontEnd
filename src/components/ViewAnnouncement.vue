      ...
      <div class="row dl-row">
        <div class="ten columns">
          ...
          <div class="card" v-for="(comment, index) in demoComments" :key="index">
            <div class="card-body">
              <p style="font-size: 14px" class="card-text">{{ comment.description }}</p>by
              <a
                href="#"
                class="card-link"
                style="color: black"
...
import { VIEW_ANNOUNCEMENT } from "../constants";
...
export default {
  ...
  data() {
    return {
      ...
      demoComments: [
        {
          description:
            "has roots in a piece of classical Latin literature from 45 BC, making it over 2000 years old. Richard McClintock, a Latin professor at Hampden-Sydne",
  ...
  mounted() {
    const url = VIEW_ANNOUNCEMENT.replace(
      "{id}",
